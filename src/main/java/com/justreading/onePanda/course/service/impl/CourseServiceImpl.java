...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements CourseService {
    ...
    public ApiResponse<List<Course>> findCourseByTermAndStudentUserName(String term, String studentUserName) {
        ...
        if (!"".equals(term) && !"".equals(studentUserName)) {
...
//                studentMethod.studentLogout(cookie);
    ...
    @Override
    public List<Course> onlyFindTermCourse(String term, String username) {
        return courseMapper.findCourseByTermAndStudentUserName(term, username);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public List<Course> refreshCourseService(RefreshCourseRequest request) {
        ...
        ApiResponse<StudentCourseDto> studentCourseDto = null;
        StudentCourseDto data = null;
        List<Course> courses = null;
        if(!ObjectUtils.isEmpty(cookie)){
            studentCourseDto = studentMethod.getStudentCourseDto(cookie, ReptileCourseOption.builder().xnxq01id(request.getTerm()).build());
            data = studentCourseDto.getData();
            studentMethod.studentLogout(cookie);
        }
        if(data != null){
            courses = CourseConverter.converterCourse(data, request.getUsername(), request.getTerm());
        }
        if (ObjectUtils.isEmpty(courses)) {
            return null;
        }else{
            List<Course> dataBaseCourses = courseMapper.findCourseByTermAndStudentUserName(request.getTerm(), request.getUsername());
            List<Integer> idList = new ArrayList<>();
            for(Course element : dataBaseCourses){
                idList.add(element.getId());
            }
            if(idList.size() != 0){
                courseMapper.deleteCourseBatch(idList);
            }
